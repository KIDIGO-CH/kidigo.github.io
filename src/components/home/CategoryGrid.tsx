      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-10">
        ...
        <div className="grid grid-cols-2 gap-2 sm:hidden">
          {categories.map((cat, i) => (
            <motion.button
              ...
              className={`group flex items-center gap-2 bg-elevated rounded-full pl-1.5 pr-3 py-1.5 border border-border hover:border-accent/30 transition-all duration-200 ${
                i >= MOBILE_INITIAL_COUNT && !expanded ? 'hidden' : 'flex'
              }`}
            ...
            >
              ...
              <span className="font-display font-semibold text-[12px] text-text-primary group-hover:text-accent transition-colors truncate">
                {cat.name}
              </span>
              <span className="text-[10px] text-text-muted flex-shrink-0">{cat.count}</span>
            </motion.button>
          ))}
        </div>
        ...
        {categories.length > MOBILE_INITIAL_COUNT && (
          <button
            ...
            className="sm:hidden flex items-center gap-1.5 mx-auto mt-3 text-[13px] font-medium text-accent hover:text-accent-light transition-colors"
          >
