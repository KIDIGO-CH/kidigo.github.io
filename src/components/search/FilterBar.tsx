...
export type DateOption = 'today' | 'weekend' | string // string = 'YYYY-MM-DD'
...
const DATE_OPTIONS: { value: 'today' | 'weekend'; label: string }[] = [
...
function Pill({ active, onClick, children, color }: { active: boolean; onClick: () => void; children: React.ReactNode; color?: string }) {
  return (
    <button
      onClick={onClick}
      className={`text-[12px] px-3 py-2 rounded-xl border transition-all duration-200 flex items-center gap-1.5 whitespace-nowrap ${
        active
      ...
      {open && (
        ...
        >
          <div className="pt-4 pb-2 space-y-3">
            ...
            {/* Row 2: Location, Age, Effort */}
            <div className="flex flex-wrap gap-2 items-center">

              {/* Indoor / Outdoor */}
              {[{ v: null as boolean | null, label: 'Tous lieux' }, { v: true, label: 'Intérieur' }, { v: false, label: 'Extérieur' }].map(({ v, label }) => (
                <Pill key={label} active={filters.indoor === v} onClick={() => set({ indoor: v })}>
                  {label}
                </Pill>
              ))}
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              {DATE_OPTIONS.map(({ value, label }) => (
                <Pill
                  ...
                  onClick={() => set({ dateFilter: filters.dateFilter === value ? null : value })}
                >
              ...
              <div className="relative">
                <input
                  ...
                  className={`text-[12px] px-3 py-2 rounded-xl border transition-all duration-200 bg-canvas border-border text-text-secondary outline-none cursor-pointer ${
                    filters.dateFilter && filters.dateFilter !== 'today' && filters.dateFilter !== 'weekend'
              ...
              {/* Clear all */}
              {Object.values(filters).some(v => v !== null && (Array.isArray(v) ? v.length > 0 : v !== defaultFilters.indoor)) && (
                <button
                  onClick={() => onChange({ ...defaultFilters })}
                  className="text-[12px] text-red-500 hover:text-red-600 flex items-center gap-1 ml-2"
                >
